 ...
 *  - 선거/임시공휴일은 제외. 필요하면 아래 Set에 날짜 한 줄 추가하면 됨.
 *  ────────────────────────────────────────────────────────────────────────── */
  ...
  return (
    <Card>
      ...
      <div className="grid grid-cols-7 gap-0.5 p-3 text-center text-xs text-gray-500">
        {["월", "화", "수", "목", "금", "토", "일"].map((w) => (
          <div key={w} className="py-2 font-semibold">
            {w}
          </div>
        ))}
      </div>
        ...
        {days.map((d, i) => {
          ...
          return (
            ...
            >
              ...
              {d && (
                <div
                  className={`mt-auto mb-1 flex flex-wrap gap-1 justify-center ${
...
function GoalsPanel({
  ...
  goalsByYear: Record<string, any[]>;
  setGoalsByYear: (v: any) => void;
  year: number;
...
function AssetsPanel({
  ...
  const [noteDown, setNoteDown] = useState("");

  // PIN
  const [pin, setPin] = useLocalStorage("pa_assets_pin", "");
  ...
  const totalsByMonth = useMemo(() => {
    ...
    monthsWindow.forEach((m) => {
      const items = assets[m]?.items || [];
      obj[m] = items.reduce((sum: number, it: any) => sum + (Number(it.amount) || 0), 0);
    });
    return obj;
  }, [assets, monthsWindow]);
  ...
  const saveNotes = () => {
    if (!month) return;
    const entry = assets[month] || { items: [] };
    setAssets({ ...assets, [month]: { ...entry, noteUp, noteDown } });
  };

  const removeItem = (m: string, id: string) => {
    const entry = assets[m];
    if (!entry) return;
    ...
    return (
      <Card>
        <div className="p-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <Button variant="secondary" onClick={setNewPin}>
            PIN 변경
          </Button>
        </div>
    ...
    <Card>
      <div className="p-4 border-b border-gray-100 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Button variant="secondary" onClick={() => shiftAnchor(-1)}>
            ◀︎
          </Button>
          ...
          <Button variant="ghost" onClick={setNewPin}>
            PIN 설정/변경
          </Button>
                ...
                <ul className="mt-2 space-y-1">
                  {r.items.map((it: any) => (
                    <li key={it.id} className="flex items-center justify-between text-sm text-gray-700">
                      ...
                      <button onClick={() => removeItem(r.month, it.id)} className="text-xs text-gray-500 hover:text-gray-800">
                        삭제
                      </button>
                    </li>
                  ))}
    ...
    <Card>
      <div className="p-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500">
          <Button variant="ghost" onClick={setNewAdminPin}>
            관리자 PIN
          </Button>
          {!adminUnlocked && (
            <>
      ...
      <header className="sticky top-0 z-10 backdrop-blur supports-[backdrop-filter]:bg-white/70 bg-white/60 border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-3 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden sm:flex items-center gap-2">
            <Button variant="ghost" href="#calendar">
              캘린더
            </Button>
            <Button variant="ghost" href="#goals">
              목표
            </Button>
            <Button variant="ghost" href="#assets">
              자산
            </Button>
            <Button variant="ghost" href="#board">
              게시판
            </Button>
          </nav>
          <div className="flex items-center gap-2">
        ...
        <div className="sm:hidden overflow-x-auto px-3 pb-2 flex gap-2">
          <Button variant="ghost" href="#calendar" className="shrink-0">
            캘린더
          </Button>
          <Button variant="ghost" href="#goals" className="shrink-0">
            목표
          </Button>
          <Button variant="ghost" href="#assets" className="shrink-0">
            자산
          </Button>
          <Button variant="ghost" href="#board" className="shrink-0">
            게시판
          </Button>
        </div>
      </header>
